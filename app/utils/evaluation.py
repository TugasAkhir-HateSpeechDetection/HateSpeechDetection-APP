...
# ============================================
# Load Model Terbaik dan Dataset Uji
# ============================================
...
def load_model_and_data():
    ...
    X = np.load(os.path.join(ROOT_DIR, 'app', 'embedded', 'bert_embedding.npy'))
    lengths = np.load(os.path.join(ROOT_DIR, 'app', 'embedded', 'bert_lengths.npy'))
    y_df = pd.read_csv(os.path.join(ROOT_DIR, 'app', 'preprocessed', 'preprocessed_data.csv'))
...
# ============================================
# Evaluasi Model secara Akurasi & Loss
# ============================================
...
# ============================================
# Laporan Klasifikasi per Label (Akurasi, Presisi, Recall)
# ============================================
...
def save_confusion_matrix(y_true, y_pred, label_names, save_path='./evaluation/confusion_matrix.png'):
    ...
    for i in range(len(label_names), len(axes)):
        fig.delaxes(axes[i])  # Hapus subplot kosong
...
def save_hamming_loss(y_true, y_pred, y_df, save_path='./evaluation/hamming_loss.json'):
    ...
    }).sort_values(by='Hamming_Loss', ascending=False)

    loss_df.to_json(save_path, orient='records')

# ============================================
# Simpan Prediksi dan Asli dalam Bentuk Excel
# ============================================
...
def save_excel_outputs(y_df, y_true, y_pred, label_names):
    ...
    hamming_loss = np.mean(y_true != y_pred, axis=1)
    loss_df = pd.DataFrame({'Tweet': tweet_data['Tweet'], 'Hamming_Loss': hamming_loss})
    ...
    y_test_df.reset_index(drop=True).to_excel('./evaluation/test_data_true.xlsx', index=False)
...
# ============================================
# Fungsi Utama Evaluasi(dipanggil dari app.py)
# ============================================
